...
try {
    ...
    if (count($data) > 0) {
        $companyName = mysql_real_escape_string($data->name, $link);
        $companyWebsite = mysql_real_escape_string($data->website, $link);

        //Call Company class to save details
        $company = new companyClass();

        $dataStatus = $company->saveCompanyDetails($companyName, $companyWebsite);
        if ($dataStatus == TRUE) {
